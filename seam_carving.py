...
def seams_removal(im, num_remove, mask=None, vis=False, rot=False):
    for i in tqdm(range(num_remove)):
        print
...
def scale_minmax(X, mi=0.0, ma=1.0):
    X_std = (X - X.min()) / (X.max() - X.min())
    X_scaled = X_std * (ma - mi) + mi
    return X_scaled

def carve_spectogram(im, dx):
    ...
    xMin = im.min()
    xMax = im.max()
    im = scale_minmax(im, 0, 255)
    output = seam_carve(im, 0, dx, None, False)
    output, _, _ = cv2.split(output)
    output = scale_minmax(output, xMin, xMax)
    return output
